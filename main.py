...
class ValueFunction:
    ...
    def learn(self, position, velocity, action, target):
        ...
        active_tiles = self.get_active_tiles(position, velocity, action)                                               
        self.weights[active_tiles] = self.weights[active_tiles] + self.step_size*(target-self.value(position, velocity, action))*1
        ##########################################################################
    ...
    while True:
        ...
        if update_time >= 0:
          ...
          for i in range(update_time+1,min(update_time+n,T)+1):
            G += ((gamma)**(i-update_time-1))*rewards[i]                                                  
            ##########################################################################
            ...
            ########################################################################## 
          if update_time+n < T:
            G += (gamma**n)*value_function.value(positions[update_time+n], velocities[update_time+n], actions[update_time+n])                                                   
            ##########################################################################
          ...
          if update_time == T-1:
            break
          value_function.learn(positions[update_time], velocities[update_time], actions[update_time], G)
...
def main():
    ...
    for run in range(runs):
        value_functions = [ValueFunction(alpha, num_of_tilings) for alpha in alphas] # two value_function
